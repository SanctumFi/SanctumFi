...
function scoreBalanceHealth(data: PlaidData): number {
  const checkingAccounts = data.accounts.filter((a) => a.type === "depository");
  if (checkingAccounts.length === 0) return 0;
  const avgBalance = checkingAccounts.reduce((sum, a) => sum + (a.balances.available || 0), 0) / checkingAccounts.length;
  const totalSpend = data.transactions.filter((t) => t.amount > 0).reduce((sum, t) => sum + t.amount, 0);
...
function scoreIncomeStability(data: PlaidData): number {
  ...
  if (mean === 0) return 0;
  const variance = months.reduce((sum, m) => sum + (m - mean) ** 2, 0) / months.length;
...
function scoreSpendingDiscipline(data: PlaidData): number {
  ...
  const essentialCategories = ["Rent", "Mortgage", "Utilities", "Groceries", "Insurance", "Transfer", "Payment", "Food and Drink"];
  const essentialSpend = spending.filter((t) => t.category?.some((c) => essentialCategories.some((e) => c.includes(e)))).reduce((sum, t) => sum + t.amount, 0);
